...
Follow the existing imperative tense, sentence-case single-line commit messages (see `git log` for examples like "Fix phantom conditional entries bug in Hammerflow submenus"). Each PR should describe motivation, approach, and testing notes; link any tracked issues or docs such as `setup.md`. Attach screenshots or short videos whenever UI changes are involved and call out new permissions required by macOS.
